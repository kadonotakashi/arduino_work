...
///  Wire.begin(SDA_OLED, SCL_OLED);
  ...
  Wire.beginTransmission(ADDRES_OLED);
    Wire.write((byte)0b00000000); //control byte, Co bit = 0 (continue), D/C# = 0 (command) Max=31byte
      Wire.write((byte)0xAE); //display off
    Wire.write((byte)0b00000000);
      Wire.write((byte)0xA8); //Set Multiplex Ratio  0xA8, 0x3F
        Wire.write((byte)0b00111111); //64MUX
    Wire.write((byte)0b00000000);    
    ...
    Wire.write((byte)0b00000000);
      Wire.write((byte)0xA1); //Set Segment re-map 0xA0/0xA1
    Wire.write((byte)0b00000000);
      Wire.write((byte)0xC8); //Set COM Output Scan Direction 0xC0,/0xC8
    Wire.write((byte)0b00000000);
      Wire.write((byte)0xDA); //Set COM Pins hardware configuration 0xDA, 0x02
        Wire.write((byte)0b00010010);
    Wire.write((byte)0b00000000);
      Wire.write((byte)0x81); //Set Contrast Control 0x81, default=0x7F
  ...
  Wire.beginTransmission(ADDRES_OLED);
    Wire.write((byte)0b00000000);
      Wire.write((byte)0x22); //Set Page Address
        ...
        Wire.write((byte)7); //End page set
    Wire.write((byte)0b00000000);
...
void Clear_Display_All(){
  ...
  for(i = 0; i < 8; i++){//Page(0-7)
    ...
    for(j = 0; j < 16; j++){//column = 8byte x 16
      Wire.beginTransmission(ADDRES_OLED);
      Wire.write((byte)0b01000000); //control byte, Co bit = 0 (continue), D/C# = 1 (data)
...
void Brightness(uint8_t brightness){
  Wire.beginTransmission(ADDRES_OLED);
    Wire.write((byte)0b00000000); //control byte, Co bit = 1 (One command only), D/C# = 0 (command)
      Wire.write((byte)0x81); //Set Contrast Control 0x81, default=0x7F
...
void Column_Page_Set(uint8_t x0, uint8_t x1, uint8_t page){
  Wire.beginTransmission(ADDRES_OLED);
    ...
    Wire.write((byte)0b00000000);
      Wire.write((byte)0x21); //set Column Address
        Wire.write((byte)x0); //Column Start Address(0-127)
        Wire.write((byte)x1); //Column Stop Address(0-127)
  Wire.endTransmission();
}
//*****************************************
